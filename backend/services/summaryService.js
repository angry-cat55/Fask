...
const executeWorkspaceSummary = async (workspaceId, userId) => {
    ...
    10. 메시지 작성자의 닉네임이 '알 수 없음'인 경우, 이는 원래 작성자가 채팅방을 나가 현재 닉네임을 확인할 수 없는 상태를 의미한다.
    11. '알 수 없음'을 실제 사용자 이름처럼 취급하거나 특정 인물로 추정하지 마라.
    12. 필요하다면 '채팅방을 나간 참여자' 또는 '퇴장한 참여자'로 표현할 수 있다.
    13. 출력은 설명 없이 요약본만 제공해.
    `;
