...
exports.saveMessage = async (workspaceId, userId, content) => {
    const sql = `
        INSERT INTO messages (workspace_id, user_id, content)
        VALUES (?, ?, ?)
    `;
    ...
    const [rows] = await pool.execute(`
        SELECT m.message_id AS messageId, m.workspace_id AS workspaceId, m.user_id AS userId, u.nickname, m.content, m.created_at AS sendAt
        FROM messages m
