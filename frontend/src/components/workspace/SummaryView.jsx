...
const markdownComponents = {
  p: ({ children }) => (
    <p className="text-base text-white leading-loose mb-3">{children}</p>
  ),
  strong: ({ children }) => (
    <strong className="font-semibold text-cyan-300">{children}</strong>
  ),
  ul: ({ children }) => (
    <ul className="list-disc pl-5 text-base text-white leading-loose mb-3">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal pl-5 text-base text-white leading-loose mb-3">
      {children}
    </ol>
  ),
    ...
    >
      {!summary ? (
        ...
        <div
          ...
          className="flex flex-1 flex-col gap-4 rounded-xl border border-white/5 bg-white/3 p-6"
        >
          <div className="flex items-center justify-between">
            <span className="text-sm text-cyan-400/70">
              #{summary.startMessageId} ~ #{summary.endMessageId}
            </span>
            <span className="text-sm text-slate-600">
              {fmt(summary.createdAt)}
            </span>
          </div>
