...
const mockInviteMember = async ({ userId = 1, invitedLoginId }) => {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      if (Math.random() < 0.15) {
        resolve({ success: false, message: '이미 초대되었거나 존재하지 않는 사용자입니다.' });
        return;
      }
      resolve({
        success: true,
            ...
            {group.items.map((item) => {
              ...
              return (
                <React.Fragment key={item.id}>
                  ...
                  >
                    <span className={highlighted ? 'text-cyan-400' : 'text-slate-600'}>
                      {item.icon}
                    </span>
                    {item.label}
                  </button>
                  ...
                  {item.id === 'members' && isMembersOpen && (
                    ...
                    >
                      <div className="space-y-3 text-xs text-slate-400">
                        <div className="text-sm font-semibold text-white">멤버 초대</div>
                          ...
                          <div className="rounded-md border border-cyan-500/20 focus-within:ring-2 focus-within:ring-cyan-400/30 p-0.5">
                            <TextInput
                              ...
                              onChange={(e) => {
                                ...
                                if (e.target.value.trim().length > 0) setInviteTouched(false);
                              }}
                              onBlur={(e) => {
                        ...
                        <div className="flex items-center justify-center gap-2 pt-1">
                          <button
                            type="button"
                            disabled={inviteLoading || inviteUserId.trim().length === 0 || !validateInviteId(inviteUserId)}
                            onClick={async () => {
                              ...
                              try {
                                ...
                                const res = await mockInviteMember({ invitedLoginId: userId });
                                if (res.success) {
                                ...
                                alert(err.message || '초대 중 오류가 발생했습니다.');
                              } finally {
                                setInviteLoading(false);
                              }
                            }}
                            className={`rounded-md px-3 py-1.5 text-xs font-medium text-white transition ${
                              inviteLoading || inviteUserId.trim().length === 0 || !validateInviteId(inviteUserId)
                          ...
                          <button
                            type="button"
                            onClick={() => { setIsMembersOpen(false); setInviteUserId(''); }}
